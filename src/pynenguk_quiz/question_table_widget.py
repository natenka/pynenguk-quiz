...
class QuestionTable(Widget):
    ...
    def render(self):
        ...
        answers_table.add_column("Номер відповіді", justify="center", no_wrap=True, vertical="middle")
        answers_table.add_column("Відповідь", justify="left")
        ...
        stats_table.add_row(
            "Правильних відповідей", f"{self.topic_stats['correct']:3}", style="black on green"
        )
        stats_table.add_row(
            "Неправильних відповідей", f"{self.topic_stats['wrong']:3}", style="black on red"
        )

        # Question table
        q_table = Table(expand=True, show_header=False, box=None)
        q_table.add_row(
            f"[green]Питання {self.current_question_number + 1} з {self.question_count}[/]\n"
    ...
    def _format_enter_number_prompt(self):
        ...
        if self.show_answer:
            enter_number_prompt = (
                f"Номер правильної відповіді: {', '.join(q_correct_answer)}"
            )
            return enter_number_prompt

        enter_number_prompt = f"Введіть номер відповіді та натисніть Enter:"
        if self.check_answer:
            enter_number_prompt = (
                f"{enter_number_prompt} {', '.join(sorted(set(self.check_answer)))} "
            )
            self.check_answer = sorted(set(self.check_answer))
            if self.check_answer == q_correct_answer:
                enter_number_prompt += "[black on green]правильна відповідь"
                self.all_stats[self.current_question_number] = True
            elif set(self.check_answer) & set(q_correct_answer):
                enter_number_prompt += "[black on yellow]частково правильна відповідь"
                self.all_stats[self.current_question_number] = False
            else:
                enter_number_prompt += "[black on red]неправильна відповідь"
                self.all_stats[self.current_question_number] = False
        return enter_number_prompt
