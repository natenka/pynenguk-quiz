...
class SelectQuestionTable(Widget):
    ...
    def render(self):
        ...
        main_table.add_row("Виберіть тему та натисніть Enter")
        main_table.add_row(topics_table)
        if self.current_topic and self.current_topic not in self.key_topic_map:
            main_table.add_row(
                f"Виберіть питання з діапазону {', '.join(self.key_topic_map)}.\n"
                f"Натисніть Enter щоб збити введені цифри",
                style="black on red",
            )
        main_table.add_row("Щоб вийти з програми, натисніть ctrl-q")
